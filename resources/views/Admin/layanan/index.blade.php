...
<div class="container mx-auto px-6 py-8 max-w-7xl space-y-10 custom-service-font overflow-hidden">
    ...
    {{-- HEADER SECTION (Match with News Architecture) --}}
    ...
    {{-- STATS CARDS (Premium Glassmorphic Layering & Staggered Micro Lift) --}}
    ...
    <div class="space-y-4">
        {{-- Pseudo Header (Hanya muncul di layar PC besar) --}}
        ...
        <div class="bg-white rounded-[2.2rem] p-5 md:p-6 shadow-[0_12px_40px_rgba(0,0,0,0.015)] border border-gray-100/80 hover:border-maroon-dark/20 hover:shadow-[0_25px_50px_rgba(159,18,57,0.06)] hover:-translate-y-2 transition-all duration-500 group relative overflow-hidden grid grid-cols-1 lg:grid-cols-12 items-center gap-6 animate-stagger-service-row" style="--row-idx: {{ $index }};">
            ...
            {{-- 1. LAYANAN INFO (Dengan Rotasi Ikon Medis Dinamis) --}}
            ...
            {{-- 3. INTERACTIVE DOCK ACTION BUTTONS (Matching Layout Berita) --}}
            <div class="col-span-1 lg:col-span-3 flex justify-end items-center gap-3 border-t lg:border-t-0 pt-4 lg:pt-0 border-gray-50">
                ...
                <a href="{{ route('admin.services.edit', $item->id) }}" 
                   ...
                   title="Edit Konfigurasi Layanan">
                    <i class="fa-regular fa-pen-to-square text-sm"></i>
                </a>

                {{-- Button Hapus / Destroy --}}
        ...
        <div class="bg-white rounded-[2.5rem] p-16 text-center border border-gray-100 shadow-sm animate-fade-up-service">
            <div class="flex flex-col items-center justify-center space-y-4">
                ...
                <div class="space-y-1">
                    ...
                    <p class="text-gray-400 max-w-xs mx-auto text-xs font-medium leading-relaxed">Sistem basis data kosong. Tekan tombol "Tambah Layanan Baru" untuk menyusun daftar poli klinik.</p>
                </div>
            </div>
        </div>
        @endforelse
...
<style>
    /* CUSTOM CORE LOGIC ENGINE ANIMATIONS */
    .custom-service-font {
        font-family: 'Plus Jakarta Sans', sans-serif;
    }

    /* 1. Global View Block Slide Animation */
    @keyframes fadeUpService {
        from { opacity: 0; transform: translateY(35px); filter: blur(6px); }
        to { opacity: 1; transform: translateY(0); filter: blur(0); }
    }
    .animate-fade-up-service {
        animation: fadeUpService 0.9s cubic-bezier(0.16, 1, 0.3, 1) forwards;
    }

    /* 2. Micro Floating White Shield Orb */
    @keyframes floatSlow {
    ...
    /* 3. Button Laser Reflection Stripe Effect */
    @keyframes laserShineStripe {
        100% { transform: translateX(100%); }
    }
    .group-hover\:animate-laser-shine {
        animation: laserShineStripe 0.9s cubic-bezier(0.25, 0.46, 0.45, 0.94);
    }

    /* 4. Cascade Waterfall Staggered Row Loading */
